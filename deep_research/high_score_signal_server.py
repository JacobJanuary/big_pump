...
# ==========================================
SCORE_THRESHOLD = 100  # Minimum score (matches composite_strategy.json)
SCORE_THRESHOLD_MAX = None  # No upper limit
TARGET_PATTERNS = ['SQUEEZE_IGNITION', 'OI_EXPLOSION']
EXCHANGE_FILTER = 'BINANCE'
EXCHANGE_IDS = {'BINANCE': 1, 'BYBIT': 2}
COOLDOWN_HOURS = 12

# Load composite_strategy.json for rule matching
STRATEGY_JSON_PATH = Path(__file__).resolve().parent.parent / 'composite_strategy.json'

def load_strategy_rules() -> List[dict]:
    """Load trading rules from composite_strategy.json"""
    try:
        with open(STRATEGY_JSON_PATH, 'r') as f:
            data = json.load(f)
            return data.get('rules', [])
    except Exception as e:
        logging.error(f"Failed to load composite_strategy.json: {e}")
        return []

def get_matching_rule(score: float, rsi: float, vol: float, oi: float, rules: List[dict]) -> Optional[dict]:
    """Find matching rule based on signal indicators"""
    for rule in rules:
        f = rule['filter']
        if f['score_min'] <= score <= f['score_max']:
            if rsi >= f['rsi_min'] and vol >= f['vol_min'] and oi >= f['oi_min']:
                return rule
    return None
...
class HighScoreSignalWebSocketServer:
    ...
    def __init__(self, config: dict):
        ...
        self.default_params = {
            ...
            'stop_loss_filter': 7.0,
            'trailing_activation_filter': 36.0,
        ...
        logger.info(f"Strategy rules loaded from: {STRATEGY_JSON_PATH}")
    ...
    def build_signal_query(self) -> str:
        ...
        # NOTE: Indicator filtering removed from SQL - done in Python via get_matching_rule()
...
"""
    ...
    async def do_full_query_and_broadcast(self):
        ...
        Deduplication: Simple 12h time-based cooldown
        """
        try:
            ...
            # Filter by composite_strategy.json rules
            rules = load_strategy_rules()
            matched_signals = []
            for sig in unique_signals:
                rsi = sig.get('rsi', 0) or 0
                vol = sig.get('volume_zscore', 0) or 0
                oi = sig.get('oi_delta_pct', 0) or 0
                rule = get_matching_rule(sig['total_score'], rsi, vol, oi, rules)
                if rule:
                    matched_signals.append(sig)
                    logger.debug(f"Signal {sig['pair_symbol']} matched rule: {rule['score_range']}")
                else:
                    logger.debug(f"Signal {sig['pair_symbol']} (score={sig['total_score']}, rsi={rsi}, vol={vol}, oi={oi}) - no matching rule")
            
            if not matched_signals:
                logger.debug("No signals matched strategy rules")
                return []
            
            # Store for new clients
            self.last_signals = matched_signals
            
            # Broadcast
            await self.broadcast_signals(matched_signals)

            logger.info(f"📡 Broadcast {len(matched_signals)} high-score signals to {len(self.authenticated_clients)} clients")
    ...
    async def broadcast_signals(self, signals: List[dict]):
        """Отправка сигналов всем аутентифицированным клиентам (simplified: pair_symbol + total_score only)"""
        if not self.authenticated_clients:
            return

        disconnected = set()
        
        # Simplified output per user requirement: only pair_symbol + total_score
        for sig in signals:
            message = json.dumps({
                'type': 'signal',
